...
delete_section_callback_data = CallbackData('delete_section', 'id')
...
inline_question_cancel = InlineKeyboardMarkup(row_width=1)
inline_question_cancel.add(InlineKeyboardButton(text='Отменить вопрос', callback_data='cancel_create_question'))
...
def generate_sections_dict():
    sections = [{'section_id': section.id, 'section_name': section.name} for section in list(Section.select().execute())]
...
def generate_sections_page(page, all_pages):
    ...
    for section in all_pages[page]:
        inline_sections_page.insert(InlineKeyboardButton(text=section['section_name'], callback_data=delete_section_callback_data.new(id=section['section_id'])))
    ...
    inline_sections_page.add(InlineKeyboardButton(text='Выйти', callback_data='exit_from_all_sections'))
